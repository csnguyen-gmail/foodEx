...
@interface FESearchPlaceSettingTVC () <UITextFieldDelegate, FEActionSheetPickerDelegate>
@end

@implementation FESearchPlaceSettingTVC
#define TYPE_STRING_LIST        @[@"Name", @"Rating", @"Most visited", @"Created date"]
...
- (FEActionSheetPicker*)buildASPFromSortString:(NSString*)sortStr withExceptTypeString:(NSString*)exceptTypeStr{
    NSMutableArray *types = [NSMutableArray arrayWithArray:TYPE_STRING_LIST];
...
- (void)setEnableSecondSort:(BOOL)enable {
    ...
    if (enable) {
        ...
        self.secondSortTF.placeholder = @"Select 1st sort condition firstly";
    }
}
